...
const { context } = defineProps<InputDatepickerProps<TApi, TDisabledDatesReq, TDisabledDatesResp>>()
...
const parentValue = computed(() => dependsOnFormValue.value?.value ?? undefined)
...
const queryFn = () => {
	return new Promise<Date[]>((resolve) => {
		...
		resolveApiEndpoint<TApi, TDisabledDatesReq, TDisabledDatesResp>(disabledDates, apiClient, request as TDisabledDatesReq).then((response: TDisabledDatesResp) => {
...
const renderInputDatepicker = () => {
	return h(DatePicker, {
		...
		date: disabledDatesQueryResult.isLoading.value || disabledDatesQueryResult.isFetching.value ? () => h(Skeleton, {
			shape: "circle",
			size: "1.5rem"
		}) : undefined
	})
