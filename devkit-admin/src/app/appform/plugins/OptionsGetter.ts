...
export const DependencyManagerPlugin: FormKitPlugin = (node) => {
	...
	if ('dependsOn' in node.props || 'requestMapper' in node.props || 'requestPropertyName' in node.props) {
		node.props.constructRequest = (): StringUnkownRecord | false => {
			const { requestMapper, requestPropertyName = 'recordId' } = node.props as DropdownContext<any, any, any>
	...
	if ('dependsOn' in node.props) {
		...
		const getParentFormValue = () => {
			...
			if (!node.parent) {
				...
				node.props.errorMessageRef.value = 'dependsOn is passed but the input has no parent'
				return false
			}
			if (!node.parent.value) return false
			if (typeof node.parent.value != 'object') return false
			const formValue = node.parent.value as StringUnkownRecord
			// Maybe you will need to use node.parent.value instead of it's clone formValue for reactivity bug
			if (!(dependsOn in formValue)) {
				node.setErrors([`dependsOn is passed but the input has no sibling with this name "${dependsOn}"`])
				return false
			}
...
export const OptionsGetterPlugin: FormKitPlugin = (node) => {
	...
	if (inputyType == 'devkitDropdown' || inputyType == 'devkitMultiDropdown') {
		...
		const { options, dependsOn, cacheKey = node.props.name, bypassCache, useLazy, optionsMapper, responseOptionsKey = 'options', cacheTimeout = 60 * 60 * 1000 * 200 } = node.props as DropdownContext<any, any, any>
		const getCacheName = () => {
		...
		const optionsFetcherFn = () => {
			const request = node.props.constructRequest ? node.props.constructRequest() : {};
			const cacheName = getCacheName()
			if (!request) return Promise.resolve([])
			return useMemoizedDropdownOptions<any, any, any>()(
				cacheName,
				cacheTimeout,
				{
					options,
		...
		const optionsFetcher = () => {
			...
			optionsFetcherFn().then(resp => {
				node.props.optionsArray.value = resp
			}).catch((e) => {
				console.log("e", e)
				if ('message' in e) {
					if (e.message.includes('is not a function')) {
						node.props.errorMessageRef.value = 'the function name passed on options not found on injected api client'
						return
					}
