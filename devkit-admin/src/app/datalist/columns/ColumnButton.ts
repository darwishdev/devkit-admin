...
export class ColumnRouter<TRecord extends Record<string, unknown>> extends ColumnBase<TRecord> implements DatalistColumnBase<TRecord> {
	paramName: string = ''
	route: string = ''
	constructor(
		name: keyof TRecord,
		...
		try {
			if (this.columnName in value) {
				...
				if (typeof currentValue == 'string') {
					const params: StringUnkownRecord = {}
