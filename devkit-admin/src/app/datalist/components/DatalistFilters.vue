...
import { type DatalistFiltersEmits, type DatalistFiltersProps, type DatalistFiltersSlots } from '../types';
...
const modelFilterFormRef = ref<Partial<Record<keyof TRecord & string, unknown>>>({})
const route = useRoute()
onMounted(() => {
  const form = localStorage.getItem(datalistStore.filtersFormKey) || RouteQueryFind(datalistStore.filtersFormKey)
...
const onFormChange = useDebounceFn(isServerSide ? (formValue) => { emit('update:modelValue', formValue) } : datalistStore.applyFilters, datalistStore.debounceInMilliSeconds)
const renderFiltersForm = () => {
  return slots.filtersForm ? slots.filtersForm(datalistStore) : h(formkitComp, {
...
const renderActiveFilters = () => {
  return ObjectKeys(datalistStore.activeFilters).map((activeFilterKey: keyof TRecord | string) => h(Chip, {
    removable: true,
...
const renderFilters = () => {
  ...
  return h(Panel, {
  ...
  },
    {
      ...
      default: () => [
        renderResetButton(), renderPresistButton(),
        renderFiltersForm()
